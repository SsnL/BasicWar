...
class NN_AI(Player):
    ...
    def __init__(self, name):
        ...
        self.regrowth_nn = self.initialize_nn(self.regrowth_nn_path, 5)
        self.ravened_nn = self.initialize_nn(self.ravened_nn_path, 5)
        self.sinkhole_nn = self.initialize_nn(self.sinkhole_nn_path, 5)
        self.daze_nn = self.initialize_nn(self.daze_nn_path, 1)

    def initialize_nn(self, file_path, output_num):
        if path.isfile(file_path):
            return pickle.load(open(file_path, "rb"))
        nn = neurolab.net.newff([[0, 10]] * 25 + [[0, 50]] * 2 + [[0, 7], [1, 60]], [3, output_num])
